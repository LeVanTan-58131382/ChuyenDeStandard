...
<div class="bill">
    ...
    <div class="bang">
        <table class="table">
            ...
            <tr>
            @foreach ($billElectric as $item)
            <?php
            if($item->paid == 0)
                {
                    {?>
                        <caption style="text-align: center; margin: 20px;"><a class="btn btn-info" href="{{route('paye', $item->user_id )}}">Khách hàng đã thanh toán</a></caption>
                    <?php }
                }
            ?> 
            @endforeach
